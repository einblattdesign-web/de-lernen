# de-lernen 🇩🇪

自分専用のドイツ語学習アプリ。Next.js + PostgreSQL (Prisma) で作られたWebアプリです。PCでローカルに動かすことも、Vercelなどにデプロイしてスマホからも使うこともできます。

## 機能

- **単語帳**: ドイツ語の単語を発音(ブラウザの音声合成)・例文・辞書的な説明つきで管理。既存のExcel(.xlsx)/CSVの単語帳から列の対応を確認しながらインポート可能。
- **フラッシュカード**: Anki風の間隔反復(SM-2アルゴリズム)で単語を復習。カテゴリ別に絞り込み可能。
- **作文問題**: Duolingo風に、英語の一文をドイツ語に書き換える問題。「B1-B2 一般語彙」「美術・デザイン語彙」の2カテゴリを用意(自由に追加可能)。
- **連続学習日数(ストリーク)**: Duolingoなど他アプリの記録を「設定」ページから引き継ぎ可能。毎日フラッシュカードか作文問題を1つ以上こなすと更新されます。
- **ストリークフリーズ**: 学習できなかった日を自動でカバーし、連続記録を守るアイテム。年6個・1年で回復。
- **コンテンツの段階的な導入**: 単語・作文問題は現在それぞれ71件ずつ用意(B1-B2一般語彙/美術・デザイン語彙 各カテゴリ約35件)。単語帳に何件あっても、フラッシュカードで一度に新規導入されるのは1日あたり指定数(初期値12件、「設定」ページで変更可)まで。作文問題は「まだやっていない/しばらくやっていない」ものから優先的に出題されるので、コンテンツを増やしても急に溢れることはありません。

## セットアップ(ローカルで動かす)

PostgreSQLのデータベースが1つ必要です。手元にPostgreSQLが無い場合は、後述の [Neon](https://neon.tech) などの無料枠を使ってもOKです(スマホ対応もするなら、どのみち後でクラウドDBが必要になります)。

```bash
npm install
cp .env.example .env   # DATABASE_URL を自分のPostgreSQL接続文字列に書き換える
npm run db:migrate     # 初回のみ: テーブルを作成
npm run db:seed        # サンプル単語・作文問題・ストリーク(1258日, 2026-09-11時点)を投入
npm run dev
```

http://localhost:3000 を開いてください。

## Excelの単語帳を取り込む

1. 「インポート」ページで `.xlsx` または `.csv` ファイルを選択して読み込む。
2. 自動で列の対応(ドイツ語・英語訳・日本語訳・品詞・性・複数形・例文など)を推測するので、間違っていれば手動で選び直す。
3. インポート先のカテゴリを選択(または新規作成)して取り込む。

列名に「ドイツ語/独語/word」「英語/english」「日本語/意味」「品詞」「性/冠詞」「複数形」「発音」「説明/定義」「例文」などの単語が含まれていると自動認識されやすくなります。ドイツ語の名詞は「性」列と「単語」列を分けて入力してください(例: 単語=`Erfahrung`, 性=`die`)。

## コンテンツの増やし方(単語・作文問題)

Excelインポートの他に、あらかじめ用意した単語・作文問題のセット(`src/lib/seedData.ts`)を投入する方法もあります。今後さらにまとめて増やしたい場合は、このリポジトリを管理しているエージェントに新しいセットの作成を依頼してください。追加後は以下のいずれかで反映します。

- ローカル: `npm run db:seed`
- 本番(デプロイ済み): デプロイ後のURLの `/api/admin/seed` にブラウザでアクセス(Basic認証あり)

`runSeed` は既存データを重複登録しない(単語は同じドイツ語表記、作文問題は同じ英文+独文の組み合わせがあればスキップ)ので、何度実行しても安全です。

新しく増えた単語・作文問題は、下記の「新規単語の導入ペース」設定に従って毎日少しずつ利用可能になります(全部が一度に出題されることはありません)。

## ストリークの引き継ぎ

「設定」ページで、現在の連続日数・最長記録・最終学習日を手動で設定できます。初期値はDuolingoの記録(2026-09-11時点で1258日)を引き継ぐようにシードしてあります。

### ストリークフリーズ

学習できなかった日があっても、ストリークフリーズを保有していれば自動的に消費されて連続記録が維持されます(何かページを開いたタイミングで自動判定されます)。

- 保有上限は6個
- 1年経過すると6個に回復(不足分を積み増すのではなく、6個にリセット)
- 保有数が足りない日数分の空白がある場合はストリークが0にリセットされます

「設定」ページでは残り個数・次回回復日を確認でき、他アプリからの引き継ぎ用に手動調整も可能です。

## スマホから使う(Vercelへのデプロイ)

このアプリをVercelにデプロイすると、発行されたURLにスマホのブラウザからアクセスするだけで、PCと同じデータ(単語帳・ストリークなど)を確認・編集できます。「ホーム画面に追加」しておくとアプリのように使えます。

### 1. データベースを用意する(Vercel Postgres / Neon)

1. [Vercel](https://vercel.com) にログインし、プロジェクトを作る前に(または作った後に)ダッシュボードの **Storage → Create Database → Postgres** から無料のPostgreSQLデータベースを作成する(内部的にはNeon)。
2. 作成すると `DATABASE_URL` などの接続情報が自動的にプロジェクトの環境変数に追加されます。

Vercelを使わずNeon単体(https://neon.tech)で作っても構いません。その場合は発行された接続文字列を後述の環境変数に手動で設定してください。

### 2. Vercelプロジェクトを作成する

1. このGitHubリポジトリ(`einblattdesign-web/de-lernen`)をVercelにインポートする(New Project → Import Git Repository)。
2. フレームワークはNext.jsとして自動検出されます。ビルドコマンドなどは変更不要です(`package.json` の `build` スクリプトが `prisma migrate deploy && next build` になっており、デプロイのたびに自動でマイグレーションが適用されます)。

### 3. 環境変数を設定する

Vercelプロジェクトの **Settings → Environment Variables** で以下を設定します。

| 変数名 | 説明 |
| --- | --- |
| `DATABASE_URL` | PostgreSQLの接続文字列(手順1で自動設定されていれば不要) |
| `BASIC_AUTH_USER` | スマホ含め外部公開する際のログインユーザー名(任意だが強く推奨) |
| `BASIC_AUTH_PASSWORD` | 同上のパスワード |

`BASIC_AUTH_USER` / `BASIC_AUTH_PASSWORD` を設定すると、アプリ全体がHTTP Basic認証で保護されます(自分のドイツ語学習データが誰でも見える状態を防ぐためです)。ローカル開発時(`npm run dev`)はこれらの環境変数を設定しなければ認証なしで使えます。

> Vercelの **Storage → Postgres** から作成した場合、変数名が `DATABASE_URL` ではなく `POSTGRES_PRISMA_URL` / `POSTGRES_URL` になっていることがあります。ビルド時に `scripts/resolve-database-url.mjs` が自動でこれらから `DATABASE_URL` を補完するので、変数名が違っていても基本的にそのまま動きます。

### 4. デプロイを実行する

Vercelは環境変数を設定しただけでは自動的にビルドし直しません。以下のいずれかでデプロイを走らせてください。

- **GitHubリポジトリ接続直後で、Deploymentsに何も表示されない場合**: まだ1度もビルドが走っていません。このブランチ(`claude/german-learning-app-jj2px5`)に何か新しいコミットをpushすると、Vercelがそれを検知して自動でビルドを開始します。
- **すでに失敗したデプロイがある場合**: Vercelダッシュボードの「Deployments」タブでそのデプロイを開き、右上の「…」メニューから **Redeploy** を選ぶと、現在の環境変数を使って再ビルドされます(コードの再pushは不要)。

しばらく待って「Deployments」タブに新しいビルドが表示され、Readyになれば完了です。

### 5. 初回シード

サンプル単語・作文問題・ストリークの初期データを本番DBにも入れたい場合は、デプロイ後のURLの `/api/admin/seed` にブラウザでアクセスしてください(例: `https://de-lernen.vercel.app/api/admin/seed`)。Basic認証のログインを求められたら設定したユーザー名・パスワードを入力します。`{"ok":true, ...}` が返れば投入完了です。

Vercelの環境変数は値がマスク表示されコピーできないため、この方法(アプリ自身が本番のDATABASE_URLを使って実行する)を使っています。既に入っているデータは重複投入されないので、何度アクセスしても安全です。

### 6. スマホから使う

デプロイ後に発行されるURL(例: `https://de-lernen.vercel.app`)にスマホのブラウザでアクセスし、Basic認証のユーザー名・パスワードを入力します。Safari/Chromeの「ホーム画面に追加」を使うとアプリのアイコンとして使えます。PCからも同じURL・同じデータベースにアクセスするので、単語帳やストリークはPCとスマホで共有されます。

## 技術構成

- Next.js (App Router) + TypeScript + Tailwind CSS
- Prisma + PostgreSQL
- exceljs によるExcel/CSVパース
- ブラウザ標準の Web Speech API による発音再生(APIキー不要)
- HTTP Basic認証(`src/proxy.ts`)による簡易アクセス制限

## よく使うコマンド

```bash
npm run dev          # 開発サーバー
npm run build         # 本番ビルド(マイグレーション適用込み)
npm run lint          # ESLint
npm run db:migrate    # マイグレーション作成/適用(ローカル開発用)
npm run db:seed       # サンプルデータ投入
npm run db:studio     # Prisma Studio (DBをGUIで確認・編集)
```
